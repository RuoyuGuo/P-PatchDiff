...
def generate_prior(img):
    img = torch.tensor(img)
    
    T,_=torch.max(img,dim=2)
    T=T+0.1
    img[:,:,0] = img[:,:,0]/ T
    img[:,:,1] = img[:,:,1]/ T
    img[:,:,2] = img[:,:,2] / T
...
def generate_coord(H, W):
    th, tw = H, W
    
    x_pos = torch.arange(tw, dtype=torch.long).unsqueeze(0).repeat(th, 1).unsqueeze(2)
    y_pos = torch.arange(th, dtype=torch.long).unsqueeze(1).repeat(1, tw).unsqueeze(2)
    
    x_pos = x_pos / (tw - 1) 
    y_pos = y_pos / (th - 1) 
    
    coord = torch.cat([x_pos, y_pos], dim=2)
