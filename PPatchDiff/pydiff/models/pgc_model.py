...
class PGCModel(BaseModel):
    ...
    def __init__(self, opt):
        super(PGCModel, self).__init__(opt)
        ...
        self.pencoder = build_network(opt['network_pencoder'])
        self.pencoder = self.model_to_device(self.pencoder)
        opt['network_ddpm']['pencoder_fn'] = self.pencoder
        
        self.colorp = build_network(opt['network_colorp'])
        self.colorp = self.model_to_device(self.colorp)
        opt['network_ddpm']['colorp_fn'] = self.colorp
        ...
        # for n,v in self.ddpm.named_parameters():
        #     print(n)
    ...
    def L1_loss(self, pred, target, weight):
    ...
    def optimize_parameters(self, current_iter):
        ...
        pred_noise, noise, x_recon_cs, x_start, t, colorp_out, colorp_target  = self.ddpm(self.HR, self.LR, 
        ...
        #prediction map loss
        l_g_c = F.l1_loss(colorp_out, colorp_target)
        loss_dict['l_g_map'] = l_g_c
        l_g_total += l_g_c
        
        #denoising loss
        l_g_noise = F.l1_loss(pred_noise, noise) #* (1 + loss_weight_map)
        # l_g_noise = self.L1_loss(pred_noise, noise, (1 + loss_weight_map)) 
    ...
    def test(self):
        if self.opt['val'].get('test_speed', False):
            ...
            with torch.no_grad():
                ...
                # LR = torch.randn(1, 8, 1920, 1080).to(self.device)
                ...
                for _ in tqdm(range(50)):
                    self.output = self.bare_model.ddim_pyramid_sample(LR, 
                                                    progressive_list=self.opt['val'].get('progressive_list'),
                                                    stride_list=self.opt['val'].get('stride_list'),
    ...
    def _initialize_best_metric_results(self, dataset_name):
        ...
        for metric, content in self.opt['val']['metrics'].items():
            # print(metric, content)
            if metric == 'psnr' or metric == 'ssim':
                init_val = float('-inf') 
            else:
                init_val = float('inf')
            record[metric] = dict(val=init_val, iter=-1)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        if with_metrics:
            ...
            self._initialize_best_metric_results('VAL')
        ...
        if with_metrics:
            for metric in self.metric_results.keys():
                ...
                self._update_best_metric_result('VAL', metric, self.metric_results[metric], current_iter)
    ...
    def _log_validation_metric_values(self, current_iter, dataset_name, tb_logger):
        ...
        for metric, value in self.metric_results.items():
            ...
            if hasattr(self, 'best_metric_results'):
                log_str += (f'\tBest: {self.best_metric_results["VAL"][metric]["val"]:.4f} @ '
                            f'{self.best_metric_results["VAL"][metric]["iter"]} iter')
            log_str += '\n'
