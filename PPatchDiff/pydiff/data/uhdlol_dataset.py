...
from scripts.utils import pad_tensor, hiseq_color_cv2_img, generate_position_encoding, generate_coord, generate_prior

@DATASET_REGISTRY.register()
class UHDLOL_Dataset(data.Dataset):

    def __init__(self, opt):
        super(UHDLOL_Dataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        #load five tensor, from low to high
#         if self.training_stage:
#             gt_map_tensor = []
#             for i in range(5):
#                 temp_tensor_mean = torch.tensor(np.load(os.path.join(self.gt_map_root, f'{gt_map_name}_s{i}.npy'))).unsqueeze(2)
#                 temp_tensor_max  = torch.tensor(np.load(os.path.join(self.gt_map_root, f'{gt_map_name}_m{i}.npy'))).unsqueeze(2)
#                 gt_map_tensor.append(temp_tensor_mean)
#                 gt_map_tensor.append(temp_tensor_max)
            
#             gt_map_tensor = torch.cat(gt_map_tensor, dim=2)
#             gt_map_tensor = gt_map_tensor.numpy()
        ...
        if self.opt.get('concat_with_hiseq', False):
            ...
            temp_lq = cv2.imread(input_path)
            temp_lq = cv2.resize(temp_lq, (w//downscale, h//downscale))
            hiseql = cv2.cvtColor(hiseq_color_cv2_img(temp_lq), cv2.COLOR_BGR2RGB) / 255.
        ...
        if self.opt.get('concat_with_prior', False):
            prior = cv2.cvtColor((cv2.imread(input_path)), cv2.COLOR_BGR2RGB)/255.
            prior = generate_prior(prior)
            input_img = np.concatenate([input_img, prior], axis=2)
...
#             input_img = np.concatenate([input_img, position_encoding], axis=2)
        ...
        #concate with attention maps
        # if self.training_stage:
        #     gt_img = np.concatenate([gt_img, gt_map_tensor], axis=2)
        ...
        if self.opt.get('concat_with_coord', False):
            H, W, _ = input_img.shape
            coord = generate_coord(H, W)
            input_img = np.concatenate([input_img, coord], axis=2)
