...
#         self.conv_gamma = nn.Conv2d(att_dim, x_dim, 3, 1, 1)
#         self.conv_beta  = nn.Conv2d(att_dim, x_dim, 3, 1, 1)
...
class PALayer(nn.Module):
    def __init__(self, att_dim, x_dim):
        ...
        self.conv_gamma_mean = nn.Conv2d(att_dim//2, x_dim, 3, 1, 1)
        self.conv_beta_mean  = nn.Conv2d(att_dim//2, x_dim, 3, 1, 1)
        
        self.conv_gamma_max = nn.Conv2d(att_dim//2, x_dim, 3, 1, 1)
        self.conv_beta_max  = nn.Conv2d(att_dim//2, x_dim, 3, 1, 1)
    ...
    def forward(self, x, p_feats):    
        ...
        #split mean max
        feats_mean, feats_max = p_feats.chunk(2, dim=1)
        
        gamma_mean = self.conv_gamma_mean(feats_mean)
        beta_mean  = self.conv_beta_mean(feats_mean)
            
        gamma_max = self.conv_gamma_max(feats_max)
        beta_max = self.conv_beta_max(feats_max)
        
        out_mean  = x * gamma_mean  + beta_mean
        out_max   = x * gamma_max   + beta_max
