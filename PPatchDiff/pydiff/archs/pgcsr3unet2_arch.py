...
class PALayer(nn.Module):
    def __init__(self, att_dim, x_dim):
        super().__init__()
        self.param_free_norm = nn.GroupNorm(32, x_dim, affine=False)
        
#         self.conv_gamma_en = nn.Conv2d((att_dim+x_dim), x_dim//2, 1)
#         self.conv_gamma_mid = nn.Conv2d(x_dim//2, x_dim//2, 3, 1, 1)
#         self.conv_gamma_de = nn.Conv2d(x_dim//2, x_dim, 1)
        
#         self.conv_beta_en  = nn.Conv2d(att_dim, x_dim//2, 1)
#         self.conv_beta_mid  = nn.Conv2d(x_dim//2, x_dim//2, 3, 1, 1)        
#         self.conv_beta_de  = nn.Conv2d(x_dim//2, x_dim, 1)

        self.conv_gamma = nn.Sequential(nn.Conv2d((att_dim+x_dim), x_dim//2, 1),
                                        nn.Conv2d(x_dim//2, x_dim//2, 3, 1, 1),
                                        nn.Conv2d(x_dim//2, x_dim, 1),
                                       )

        self.conv_beta = nn.Sequential(nn.Conv2d((att_dim+x_dim), x_dim//2, 1),
                                nn.Conv2d(x_dim//2, x_dim//2, 3, 1, 1),
                                nn.Conv2d(x_dim//2, x_dim, 1),
                               )
    ...
    def forward(self, x, p_feats):    
        normalized = self.param_free_norm(x)
        assert x.size()[2:] == p_feats.size()[2:]
        
        # p_feats = self.sp(p_feats)
        
        gamma = self.conv_gamma(torch.cat([x, p_feats], dim=1))
        beta = self.conv_beta(torch.cat([x, p_feats], dim=1))
        out = x * gamma  + beta
...
#         self.conv_init  = nn.Sequential(nn.Conv2d(att_dim, att_dim, 1),
#                                         nn.ReLU())
        
#         self.conv_gamma = nn.Sequential(nn.Conv2d(att_dim, x_dim//2, 1),
#                                         nn.Conv2d(x_dim//2, x_dim//2, 1),
#                                         nn.Conv2d(x_dim//2, x_dim, 1),
#                                        )
        
#         self.conv_beta  = nn.Sequential(nn.Conv2d(att_dim, x_dim//2, 1),
#                                         nn.Conv2d(x_dim//2, x_dim//2, 1),
#                                         nn.Conv2d(x_dim//2, x_dim, 1),
#                                        )
...
#         # p_feats = self.sp(p_feats)
#         p_feats = self.conv_init(torch.mean(p_feats, dim=[2, 3], keepdim=True))
        
#         gamma = self.conv_gamma(p_feats)
#         beta = self.conv_beta(p_feats)
#         out = x * gamma  + beta
...
class ResnetBlocWithAttn(nn.Module):
    def __init__(self, dim, dim_out, *, noise_level_emb_dim=None, norm_groups=32, dropout=0, \
        with_attn=False, use_affine_level=False, use_CA=False, attn_type=None, use_PA=False):
        super().__init__()
        ...
        self.use_PA = use_PA
        if use_PA:
            self.pa = PALayer(att_dim=dim_out, x_dim=dim)
        

    def forward(self, x, time_emb, p_feats=None):
        if self.use_PA and p_feats is not None:
            x = self.pa(x, p_feats)
            # print('###############################')
...
class PGCSR3UNet2(nn.Module):
            ...
            for _ in range(0, res_blocks):
                downs.append(ResnetBlocWithAttn(
                    pre_channel, channel_mult, noise_level_emb_dim=noise_level_channel, norm_groups=norm_groups, dropout=dropout, with_attn=use_attn, use_affine_level=use_affine_level, \
                        use_CA=use_CA, attn_type=attn_type, use_PA=True))
    ...
    def forward(self, x, time=None, att_feats=None):
        ...
        for i, layer in enumerate(self.downs):
            if isinstance(layer, ResnetBlocWithAttn):
                x = layer(x, t, att_feats[maps_feats_counts])
                maps_feats_counts += 1
            else:
                x = layer(x)
            feats.append(x)
